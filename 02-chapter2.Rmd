...
An important example for this is the CLIP model (Radford, Kim et al., 2021) with its new dataset WIT (WebImageText) comprising 400 million text-image pairs scraped from the internet. \
Similar to "Text2Image" the recent successes in NLP have inspired new approaches in this field.
Most importantly pre-train methods, which directly learn from raw text (e. g. GPT-n; Generative Pre-trained Transformer; Brown et al., 2020).
...
Like described in previous parts, devoid of human labelling, self-supervised models don't suffer from the same capacity constraints as regular supervised learning models.
...
Hence, they see more and use more.
...
As a result of the use of a continuous and contextualized target, the representations achieved via self-attention are richer than a fixed set of targets based on local context as used in most prior work.
