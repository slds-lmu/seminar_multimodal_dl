...
The first part “Image2Text” discusses how transformer-based architectures improve meaningful captioning for complex images using a new large scale, richly annotated dataset COCO.
...
In 2014 Microsoft COCO was developed with a primary goal of advancing the state-of-the-art (SOTA) in object recognition by diving deeper into a broader question of scene understanding.
...
With the help of the COCO dataset, a transformer-based architecture: Meshed-Memory Transformer for Image Captioning ($M^2$) will be introduced to improve both image encoding and the language generation steps.
...
Current advancements in this field have been made possible largely due to recent breakthroughs in NLP, which firstly allowed for learning contextual representations of text.
...
The first model is DALL-E (Ramesh et. al, 2021), which essentially combines Variational Encoder (VAE) with Autoregressive Transformer.
...
The next approach to text-to-image generation is a GLIDE model (Nichol et. al, 2021).
...
Afterwards, in the part “Text supporting Image Models”, approaches where natural language is used as supervision for CV models are described.
...
An important example for this is the CLIP model (Radford, Kim et al., 2021), with its new WIT (WebImageText) dataset comprising 400 million text-image pairs scraped from the internet.
Similar to "Text2Image" the recent successes in NLP have inspired new approaches in this field.
Most importantly their pre-train methods, which directly learn from raw text (e. g. GPT-n; Generative Pre-trained Transformer; Brown et al., 2020).
...
Like in previous parts, devoid of human labelling, self-supervised models don't suffer from the same capacity constraints as regular supervised learning models.
...
Rather than potentially learning modality-specific biases, a general multipurpose framework can help to increase robustness while also simplifying the learner portfolio and thereby better emulating human learning processes. \
...
The core idea of data2vec, developed by MetaAI, is to predict latent representations of the full input data based on a masked view of the input in a self-distillation setup using a standard transformer architecture (Baevski et al, 2022).
...
Hence, they see more and more use.
...
Normalization helps to prevent that, as well as the domination of certain layers with high norm.
