...
@shen2021much study how the performance of Vision-and-Language (V&L) models improves, when the visual encoder is switched to CLIPs strong image encoder.
...
They reiterate the necessity of large-scale vision datasets, but assert that even CLIPs data collection process still involves a non-trivial amount of data curation.
...
This means arithmetic operations like the ones mentioned in chapter \@ref(sotanlp) are not only meaningful on encodings belonging to the same modality, but to different. \
...
```
(ref:img-txt-addition) Multimodal image retrieval via arithmetic operation on word and image embeddings.
...
For the two encoders of the pre-trained core they use a hierarchical Vision Transformer (CoSwin Transformer) for images and a Transformer similar to CLIPs for text.
...
Table \@ref(fig:table1) summarizes the zero-shot accuracies on different ImageNet variants.
...
Even tough there are not many major difference in scores, there is a clear order with CLIP on third, ALIGN on second and Florence on first place.
...
The most comprehensive comparison comprises table \@ref(fig:table3).
It depicts the accuracy of zero-shot CLIP and Florence on various datasets as well as the scores of all three models fine tuned.
For zero-shooting Florence beats CLIP in nearly all evaluations, as well as for fine tuned performance.
@jia2021scaling only report on three of the twelve dataset, where they lead two thirds of the time.
...
CLIP continues to be used inside new models, e.g., DALL$\cdot$E 2, where it embeds images [@ramesh2022hierarchical]. \
Also, there is an effort to replicate CLIP training dataset called LAION-400M dataset [@schuhmann2022laion].
