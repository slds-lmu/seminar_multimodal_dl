...
$\ell_1^{V_\text{img}, V_\text{txt}}$ is the loss for the first image-text pair, $v_\text{img}^1$ and $v_\text{txt}^1$ are the respective latent representations and $h_\theta(\cdot)$ is a similarity measure. \
Figure \@ref(fig:contr-viz) further visualizes the concept.
It can be seen, how the images and the texts are encoded by their respective sub-networks, resulting in a matrix with elements $h_\theta(\{v_\text{img}^i,v_\text{txt}^j\})$.
Loosely speaking, the contrastive objective is to get the diagonal elements to, e.g., 1 and the rest to 0.
...
<!-- CONTINUE: training in latent space vs target space -->
