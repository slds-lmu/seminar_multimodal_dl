...
In order to guarantee symmetry, the total loss is the sum of $\ell_1^{V_\text{img}, V_\text{txt}}$ and $\ell_1^{V_\text{txt}, V_\text{img}}$, where the similarity of one text to all images is averaged instead of the other way around.
...
Figure \@ref(fig:contr-vs-pred-learn) gives another interesting insight into contrastive learning, when it is compared to classical predictive learning.
...
Conversely, approach (b) measures the loss in the representation space.^[Note that contrastive learning easily works with different combinations of modalities, here B&W and colors.]
...
But, rather than theoretical considerations, the driving factor for using this objective function is data efficiency.
As can be seen in figure \@ref(fig:data-efficiency), @radford2021learning started their search for an adequate pre-train model (more on this in subsection \@ref(foundMod)) by experimenting with a transformer based language model predicting the exact captions of an image.
This turns out to train three times slower, in terms of data efficiency, compared to a simpler baseline predicting a bag-of-words encoding.
