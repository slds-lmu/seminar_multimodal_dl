...
experts in the MoE layers were used. Compared to training Multimodel for each task specifically, the combined training showed better performance. 
...
since few-shot learning is not feasible due to the input length restrictions in transformers. However, the results were mixed, so improvements were possible, the expert was
outperformed, and even massive fine-tuning efforts only led to small gains. In robotics, GATO showed similar behavior to the baseline sota model. Additionally, GATO also showed 
