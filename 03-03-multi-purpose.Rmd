...
#### LIMoE
Another work, worth mentioning in the context of the pathway proposal, is the LIMoE model [@Mustafa2022], which combines text and vision as modalities in a mixture-of-expert
model.
- Description
- architectural (transformer exactly, buffer, contrastive)
The model described in LIMoE is also transformer-based. To provide the input appropriately, the text data is encoded using one-hot-SentencePiece encoding, while the images are
tokenized in the same way, as done in ViT [@dosovitskiy2020image]. The output of the transformer encoder is then fed into the mixture-of-expert layers, which consists of $E$ 
experts from which $K$ experts are used in a forward pass. The decision about the experts is done by using a router layer which decides where the tokens are passedpm to using
top-k routing. 
- Objective
...
one modality. To address this issue two new losses based on entropy are introduced. The losses are aimed at controling the allocation of experts to tokens.
