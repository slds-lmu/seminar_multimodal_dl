...
#### PathNet
A similar approach, however neither multipurpose nor using specialist networks, is the model Pathnet [@Fernando2017]. Pathnet entails a concept for sequential multi-task learning
and furthermore an indirect neural architecture search. Pathnet is build upon a evolutianary algorithm. Evolutionary algorithms are based on trial and error to find an optimum and are 
can thus be used for optimization. An initial population of values is spawned, from which then a children generation is constructed. This construction can be done by combining individuals
from the initial population (parents), based on their fitness (how good they are for solving the problem). The children from the earlier step become the parents in the next generation and the procedure
is repeated until convergence. For a more detailed overview, the work of (X) is recommended. Due to the non-differentiable propoerty of EAs, they become an ideal algorithm for hyperparameter tuning
or neural architecture search.
Pathnet applies EAs to find the best paths through a greater neural network, that is analogous to the one proposed by [@Dean21]. A directed graph, mapping from the input to the output
where layers with nodes are in between. The nodes are also neural networks here. At first a population of paths is spawned, mapping input to output through the overall neural network.
...
#### muNet
While LIMoE offered a new appraoch to multimodality, another model, named muNet [@Gesmundo2022a], aims at solving multi-tasking. Asit was already described in the initial pathways proposal, many aspects of current
...
By applying this technique, the model evolves from an initial proposal to a suited multit-task model.
- evolutianary algo
The algorithm to develop the eventual model consists of three main stages, these stages include at first the selection of parent models which are mutated, the actual mutation step, and the tournament after training
the child models on a specific task.
In the beginning a root model is used, from which the further model is evolved from. This model can be a randomly initialized model or a pre-trained model. During training the so-called active population is evolveed for each
active task (which can be active multiple times), which includes the candidates for mutation.
Sampling appropriate parents is done by computing draw probabilities of available models. If models have already been trained on this task and have been evaluated, then it is possible to take the success already into account.
The models are ordered in an asceding order with respect to the previously obtained score, then a probability is computed which also takes into account how often these models where already mutated: $\mathbb P(m|t) = 0.5 ^{ \#timesSelected(m, t)}$.
where $\#timesSelected(m, t)$ is the number of previous mutations based on this model. The resulting probability thus considers exploitation (the earlier the model is called, the more likely it becomes that an event is successful based on the ordering of the list)
and also exploration by considering the previous amount of mutation, thus punishing ineffective canidates which have not had successful offspring and improving the draw probability of fewer mutated candidates. If this does not succeed, models are randomly
chosen from a list of all models including also the inactive ones, which are also collected for other tasks. For the start of the algorithm, the initially proposed model is used.
The next step includes mutating the sampled canidates. Since this step is also random, it is thus not an issue if the same model has been drawn multiple times, since the probability of the exact same mutation is relatively low.
In this step, the mutations are applied to the model and it is randomly decided wether or not the mutation is accepted. Afterwards, the models are trained on the task and scored. The best child model is kept and later also compared against
already saved models, where also only the best model is kept.
- Mutations
Evolutionary algorithms require methods to mutate the initial parent population such that new modificiations (children) are obtained, from which a winning candidate is then chosen for the next generation. The mutations for muNet include four
procedures to mutate models. These include:
 - Layer cloning, in which the child model consists of the frozen parent model with an additional modifyable layer. This layer is a copy of the original layer from the parent, so that the already acquired knowledge is not lost but can be enhanced.
   The child model still uses all other layers from the parent. The only further exception is in case of another task the model is trained on. In this case, the task head is also newly trained.
 - Layer insertion, where two layers are added to the model to be mutated. These layers are residual adapters based on (X) and (X), where the dimension inbetween can be arbitrary and the layers are newly initialized. For training, the second layer is initialized using zeros, such that
   an identity mapping is used first, so that the knowledge of the previously trained model is still used. During training, task specific knowledge can be acquired, based on the already trained starting point.
 - Layer removal is used to skip layers from the parent model. The other layers are still in use being frozen. For transformers, it was specifically designed, however, that only the latter layers can be removed.
 - Hyperparameter change, which is a random sampling of hyperparameters close to the current ones in use. This is done by sampling from a list of neighboring hyperparameters.

- Results
The model was furthermore evaluated on two challenges for finr-tuning in a multitask setting in the field of computervision. The first was the character mutlitask "Multitask Character Classification Benchmark" and the second one was the
"Decathlon". The baseline was a ViT specifically designed to be genralizable on multiple tasks. Overall muNet outperformed ViT in almost all tasks on both benchmarks.
Furthermore another itertion of muNet, which was just published shortly afterwards [@], was also tested. This version does not include layer removal and layer insertion for mutations but also includes the acceptance probabilty for mutations as a evolvable
hyperparameter. An ablation study showed that the performance increases further when leaving layer insertion out [@Gesmundo2022a]. This model was also evalutated on the same benchmarks and also achieved similar results.

- Aspect about multitask learning. 
muNet tries to overcome prominent hindrances in multitask learning. Through freezing the parameters of the parent in the children model, it is possible to avoid catastrophic forgetting. Furthermore the issue of negative transfer is also
...
While LIMoE proved that multimodality works in a sparse context, muNet achieved a neural architecture search in a multitask context. Both models are not the implementation of the pathway proposal but they may pave the path further. Both models are not multi-purpose, but it might
be interesting to combine these approaches further down the road to even better muli-purpose models.
