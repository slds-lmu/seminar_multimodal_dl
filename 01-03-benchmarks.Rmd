...
SSL leads to good zero-shot results [align paper, clip paper, parti paper, imagen paper], because through SSL the models learn a good representation of the given input. Zero-shot learning (ZSL) is a problem in machine learning, where during test time, a learner observes samples from classes not observed during training when the learner needs to predict sample class. This shows that the models can achieve good results even without fine-tuning, but fine-tuned models achieve even better results. The fine-tuning procedure naturally depends on the specific down-task. For example for image classification it is possible to freeze the learned visual features and only train a classification head or fine-tune all layers. This chapter gives just give an example of fine-tuning as these fine-tuning procedures differ very much depending on the modality.



After pre-training and also fine-tuning the models there is a need to compare the models. This need lead to the create of dataset for test purpose often called benchmarks. 
  ...
  \begin{itemize}
    ...
    \item How has pre-training changed or has it even changed in modalities (DONE)
      ...
      \end{itemize}
      \item MML self-supervised learning
        \begin{itemize}
          \item BLIP
          \item CoCa
        \end{itemize}
